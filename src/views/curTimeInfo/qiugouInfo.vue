...
export default {
  ...
  methods: {
    ...
    delectCom(){
      this.$fetchPost("purchaseDemand/delete",{id:this.curId}).then(res => {
        this.$message({
          message: res.message,
          type: 'success'
        });
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (this.addOrEditPoint==0){
            this.$fetchPost("purchaseDemand/insert",this.form,"json").then(res => {
              this.$message({
                message: res.message,
                type: 'success'
              });
    ...
    getNeedsNameList(){
      this.$fetchGet("purchaseDemand/getTitle").then(res => {
    ...
    getTableData(params){
      this.$fetchGet("purchaseDemand/getList",params).then(res => {
